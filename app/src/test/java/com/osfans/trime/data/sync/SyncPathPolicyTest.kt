...
class SyncPathPolicyTest :
    StringSpec({
        ...
        "preserve own sync folder when ownId is known" {
            ...
            SyncPathPolicy.shouldPreserveLocal("sync/phone-a/luna.userdb.txt", null) shouldBe true
        }
        ...
        "installation id must be one path component" {
            for (id in listOf(".", "..", "../peer", "peer/child", "peer\\child", "/peer")) {
                SyncPathPolicy.readInstallationId("installation_id: '$id'") shouldBe null
                shouldThrow<IllegalArgumentException> { SyncPathPolicy.ownSyncPrefix(id) }
                shouldThrow<IllegalArgumentException> { SyncPathPolicy.localOwnSyncDir(id, "sync", File("user")) }
            }
        }
        "unknown identity preserves custom sync dumps but still imports peers" {
            SyncPathPolicy.shouldPreserveLocal("backups/phone-a/luna.userdb.txt", null, "backups") shouldBe true
            SyncPathPolicy.shouldPreserveLocal("backups-extra/phone-a/dump.txt", null, "backups") shouldBe false
            SyncPathPolicy.shouldImport("backups/phone-b/luna.userdb.txt", null, "backups") shouldBe true
        }
        "own sync directory cannot follow a symlink outside the sync root" {
            val root = createTempDirectory().toFile()
            val outside = createTempDirectory().toFile()
            try {
                val sync = File(root, "sync").apply { mkdirs() }
                java.nio.file.Files.createSymbolicLink(File(sync, "phone-a").toPath(), outside.toPath())
                shouldThrow<SyncRelativePath.PathEscapeException> {
                    SyncPathPolicy.localOwnSyncDir("phone-a", "sync", root)
                }
            } finally {
